...
async function updateUnreadCounts() {
    ...
    try {
        const res = await fetch(`/api/chat/unread/trainer/${currentTrainerId}`, {
...
async function markAsRead(relId) {
    try {
        await fetch(`/api/chat/${relId}/read?readerType=TRAINER`, {
            method: 'PUT',
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') }
        });
        const badge = document.getElementById(`badge-${relId}`);
...
async function loadClients() {
    ...
    try {
        const res = await fetch(`/api/trainer-client/clients/${currentTrainerId}`, {
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') }
        });
...
async function loadPendingRequests() {
    ...
    try {
        const res = await fetch(`/api/trainer-client/requests/${currentTrainerId}`, {
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') }
        });
...
async function respondRequest(requestId, status) {
    try {
        const res = await fetch(`/api/trainer-client/respond?requestId=${requestId}&status=${status}`, {
            method: 'POST',
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') }
        });
        if (res.ok) {
...
async function openWorkoutModal(relId, name, readOnly = false) {
    ...
    if (readOnly) {
        ...
        // Fetch
        try {
            const res = await fetch(`/api/plans/workout/${relId}`, {
                headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') }
            });
            if (res.ok) {
...
async function openDietModal(relId, name, readOnly = false) {
    ...
    if (readOnly) {
        ...
        try {
            const res = await fetch(`/api/plans/meal/${relId}`, {
                headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') }
            });
            if (res.ok) {
                const plans = await res.json();
...
async function loadTrainerChat() {
    ...
    try {
        const res = await fetch(`/api/chat/${activeRelationshipId}`, {
            headers: { "Authorization": "Bearer " + localStorage.getItem('token') }
        });
        if (res.ok) {
...
async function submitAddClient() {
    ...
    try {
        const res = await fetch(`/api/trainer-client/add?trainerId=${currentTrainerId}&userEmail=${email}`, {
            method: 'POST',
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') }
        });
